...
abstract contract MortgageBase is Member {
    ...
    function _mortgage(address owner, int256 amount) internal {
        ...
        if (_now > startTime && totalAmount != 0) {
            int256 reward;
            if (_now < startTime.add(totalDuration)) {
                reward = int256(totalReward * (_now.sub(startTime)) / totalDuration)
                    + totalAdjust;
            } else {
    ...
    function stopMortgage() external CheckPermit("Admin") {
        uint256 _now = block.timestamp;
        require(_now < startTime.add(totalDuration), "mortgage over");
        ...
        if (_now < startTime) {
            ...
            uint256 reward = totalReward.mul(_now.sub(startTime)).div(totalDuration);
            tokenAmount = totalReward.sub(reward);
            totalReward = reward;
            totalDuration = _now.sub(startTime);
        }
