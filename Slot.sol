...
contract Slot is Member, IERC721TokenReceiverEx {
    ...
    function _onFightChanged(address owner) internal {
        ...
        int256 buffer = 0;
        if (ui.periodCount[3] >= 1) {
            buffer = countBuffers[3];
        } else if (ui.periodCount[2] >= 2) {
            buffer = countBuffers[2];
        } else if (ui.periodCount[1] >= 3) {
            buffer = countBuffers[1];
        } else if (ui.periodCount[0] >= 5) {
            buffer = countBuffers[0];
        }
    ...
    function _addCards(address owner, uint256[] memory cardIds) internal {
        ...
        for (uint256 i = ui.periodCount.length; i < 4; ++i) {
            ui.periodCount.push(0);
        }
    ...
    function removeCard(uint256 cardType) external {
        ...
        si.cardId = 0;
        ui.count--;
        if (cardType <= 10 && ui.periodCount[0] > 0) {
            ui.periodCount[0]--;
        } else if (cardType <= 25 && ui.periodCount[1] > 0) {
            ui.periodCount[1]--;
        } else if (cardType <= 35 && ui.periodCount[2] > 0) {
            ui.periodCount[2]--;
        } else if (cardType > 35 && ui.periodCount[3] > 0) {
            ui.periodCount[3]--;
        }
    ...
    function upgrade(address owner, uint256[] memory cardIds) external {
        ...
        for (uint256 i = 0; i != cardIdLength; ++i) {
            ...
            si.exp = si.exp.add(rarityExps[uint16(cardId >> 192)]);
